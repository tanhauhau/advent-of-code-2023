const input = `Game 1: 4 red, 8 green; 8 green, 6 red; 13 red, 8 green; 2 blue, 4 red, 4 green
...
let sum = 0;
game: for (const line of input.split('\n')) {
  const [_, game, turns] = line.match(/^Game (\d+): (.+)$/);

  const minimumAmount = { red: 0, green: 0, blue: 0 };
  for (const turn of turns.split('; ')) {
    for (const pick of turn.split(', ')) {
      const [_, amount, color] = pick.match(/(\d+) (.+)/);
      minimumAmount[color] = Math.max(minimumAmount[color], Number(amount))
    }
  }

  sum += minimumAmount.red * minimumAmount.blue * minimumAmount.green;
}
