    ...
    if (map[row][column] === '*') {
      ...
      for (const [rowToSearch, columnToSearch] of [
        ...
        if (
          ...
          /[0-9]/.test(map[rowToSearch][columnToSearch])
        ) {
          count++;
...
function isWithinBoundary(row, column) {
  return (
...
function getNumber(row, column) {
  ...
  while (startColumn > 0 && /[0-9]/.test(map[row][startColumn - 1])) {
    startColumn--;
    visited.add(`${row},${startColumn}`);
  }

  while (
    endColumn < map[row].length - 1 &&
    /[0-9]/.test(map[row][endColumn + 1])
  ) {
    endColumn++;
    visited.add(`${row},${endColumn}`);
  }

  return Number(map[row].slice(startColumn, endColumn + 1));
}
