...
let finalCalibrationValue = 0;
for (const line of input.split('\n')) {
  let firstNumber;
  let lastNumber;

  searchFirstNumber: for (let i = 0; i < line.length; i++) {
    const char = line[i];
    if (/[0-9]/.test(char)) {
      firstNumber = Number(char);
      break;
    } else {
      for (let j = 0, value = 1; j < digits.length; j++, value++) {
        if (line.slice(i).startsWith(digits[j])) {
          firstNumber = Number(value);
          break searchFirstNumber;
        }
      }
    }
  }
  searchLastNumber: for (let i = line.length - 1; i >= 0; i--) {
    const char = line[i];
    if (/[0-9]/.test(char)) {
      ...
      for (let j = 0, value = 1; j < digits.length; j++, value++) {
        if (line.slice(0, i + 1).endsWith(digits[j])) {
          lastNumber = Number(value);
          break searchLastNumber;
        }
      }
    }
  }
  ...
  finalCalibrationValue += calibrationValue;
}

console.log(finalCalibrationValue);
